...
const { determinarCargoAuto } = require('../utils/cargoHelper');
...
router.get('/tickets', async (req, res) => {
    ...
    try {
        ...
        let matriculaKey = matricula;
        if (matricula) {
            const cargoLogado = await determinarCargoAuto(conn, matricula);
            matriculaKey = String(matricula) + ':' + cargoLogado;
        }

        // Ordenacao:
        // 1o - Tickets referenciados/mencionados AO USUARIO LOGADO (so aparece se o usuario tiver)
        // 2o - Filtrar TODOS: ativos primeiro (ABERTO/EM ATENDIMENTO), finalizados/fechados no final
        // 3o - Dentro de cada grupo: ticket aberto ha mais tempo primeiro (CRIADO_EM DESC)
        if (status === 'TODOS') {
            binds.matriculaKey = matriculaKey;
            sql += ` ORDER BY CASE WHEN t.MENTIONED_MATRICULA = :matriculaKey THEN 0 ELSE 1 END,
                      CASE WHEN t.STATUS IN ('ABERTO', 'EM ATENDIMENTO') THEN 0 ELSE 1 END,
                      t.CRIADO_EM DESC`;
        } else if (matricula) {
            binds.ord = matriculaKey;
            sql += ` ORDER BY CASE WHEN t.MENTIONED_MATRICULA = :ord THEN 0 ELSE 1 END, t.CRIADO_EM DESC`;
        } else {
...
router.post('/tickets/:id/reply', async (req, res) => {
    ...
    try {
        ...
        const codcliTicket = ticketRes.rows[0][5];
        ...
        // 3b. Envia também para o VENDEDOR que atende o cliente (Task 3)
        if (!isInternal && codcliTicket) {
            try {
                const vendRes = await conn.execute(`SELECT CODUSUR1 FROM PCCLIENT WHERE CODCLI = :c AND ROWNUM = 1`, { c: codcliTicket });
                if (vendRes.rows.length > 0 && vendRes.rows[0][0]) {
                    const vendCodusur = String(vendRes.rows[0][0]);
                    let vendTel = null;
                    const usrRes = await conn.execute(`SELECT TELEFONE1 FROM PCUSUARI WHERE CODUSUR = :c AND ROWNUM = 1`, { c: vendCodusur });
                    if (usrRes.rows.length > 0 && usrRes.rows[0][0]) vendTel = usrRes.rows[0][0];
                    if (!vendTel) {
                        const empRes = await conn.execute(`SELECT CELULAR FROM PCEMPR WHERE MATRICULA = :c AND ROWNUM = 1`, { c: vendCodusur });
                        if (empRes.rows.length > 0 && empRes.rows[0][0]) vendTel = empRes.rows[0][0];
                    }
                    const vendInst = await conn.execute(`SELECT INSTANCE_NAME, API_TOKEN, COALESCE(API_URL, (SELECT VALOR FROM CANAL_CONFIGURACOES WHERE CHAVE = 'EVOLUTION_API_URL')) FROM CANAL_TOKENS_EVOLUTION WHERE CODUSUR = :c`, { c: vendCodusur });
                    
                    if (vendTel && vendInst.rows.length > 0) {
                        const vInst = vendInst.rows[0][0];
                        const vTok = vendInst.rows[0][1];
                        const vUrl = vendInst.rows[0][2];
                        const vMsg = `*Ticket #${id} - ${titulo}*\n\n${finalMessage}`;
                        
                        // Envia para o Vendedor
                        try {
                            await axios.post(`${vUrl}/message/sendText/${vInst}`, { number: telFormatado(vendTel), text: vMsg }, { headers: { 'apikey': vTok, 'Content-Type': 'application/json' }, timeout: 8000 });
                        } catch (e) {
                            try {
                                await axios.post(`${vUrl}/send/text`, { number: telFormatado(vendTel), text: vMsg }, { headers: { 'apikey': vTok, 'instance': vInst, 'Content-Type': 'application/json' }, timeout: 8000 });
                            } catch (e2) { console.error('[SAC] Erro ao notificar vendedor (Evo Go):', e2.response?.data || e2.message); }
                        }
                    }
                }
            } catch (e) {
                console.error('[SAC] Erro ao notificar vendedor do cliente:', e.message);
            }
        }
...
router.post('/tickets/:id/send-media', upload.single('file'), async (req, res) => {
    ...
    try {
        ...
        // 1. Encontra ticket
        const ticketRes = await conn.execute(`
            SELECT t.TELEFONE, t.STATUS, d.NOME, p.NOME, t.CODCLI
            FROM CANAL_SAC_TICKETS t
            LEFT JOIN CANAL_SAC_DEPARTAMENTOS d ON t.DEPARTAMENTO_ID = d.ID
            LEFT JOIN CANAL_SAC_DEPARTAMENTOS p ON d.DEPARTAMENTO_PAI_ID = p.ID
            WHERE t.ID = :id
        ...
        const deptoNome = ticketRes.rows[0][2];
        const paiNome = ticketRes.rows[0][3];
        const codcliTicket = ticketRes.rows[0][4];
        ...
        // 6b. Notifica o VENDEDOR que atende o cliente (Task 3)
        if (codcliTicket) {
            try {
                const vendRes = await conn.execute(`SELECT CODUSUR1 FROM PCCLIENT WHERE CODCLI = :c AND ROWNUM = 1`, { c: codcliTicket });
                if (vendRes.rows.length > 0 && vendRes.rows[0][0]) {
                    ...
                    if (vendTel && vendInst.rows.length > 0) {
                        ...
                        const vTok = vendInst.rows[0][1];
                        const vUrl = vendInst.rows[0][2];
                        const vMsg = `*Notificação de Ticket #${id}*\n${titulo}\n\n📎 *Anexo enviado ao cliente:* ${fileName}\n*Atendente:* ${attendantName || 'SAC'}`;
                        try {
                            await axios.post(`${vUrl}/message/sendText/${vInst}`, { number: telFormat(vendTel), text: vMsg }, { headers: { 'apikey': vTok, 'Content-Type': 'application/json' }, timeout: 8000 });
                        } catch (e) {
                            try {
                                await axios.post(`${vUrl}/send/text`, { number: telFormat(vendTel), text: vMsg }, { headers: { 'apikey': vTok, 'instance': vInst, 'Content-Type': 'application/json' }, timeout: 8000 });
                            } catch (e2) { console.error('[SAC] Erro ao notificar vendedor (Evo Go):', e2.response?.data || e2.message); }
                        }
                    }
                }
            } catch (e) {
                console.error('[SAC] Erro ao notificar vendedor do cliente (media):', e.message);
            }
        }
...
router.get('/clientes/:codcli', async (req, res) => {
    ...
    try {
        ...
        conn = await oraclePool.getConnection();
        const result = await conn.execute(`
            SELECT CLIENTE, FANTASIA, TELCELENT, TELCOB, TELENT, TELCOM
            FROM PCCLIENT
            WHERE CODCLI = :codcli
        `, { codcli });
        ...
        let telefone = result.rows[0][2] || result.rows[0][3] || result.rows[0][4] || result.rows[0][5] || '';
        if (telefone) {
    ...
    try {
        ...
        if (instRes.rows.length > 0) {
            ...
            if (file) {
                ...
                // SEMPRE salva o anexo e o disponibiliza no ticket, mesmo que o envio via WhatsApp falhe
                let subFolder = 'Documentos';
                if (mediatype === 'image') subFolder = 'Imagens';
                else if (mediatype === 'video') subFolder = 'Video';
                else if (mediatype === 'audio') subFolder = 'Audio';
                ...
                try {
                    fs.writeFileSync(filePath, file.buffer);
                    mediaUrl = `/SAC/UPLOAD/${subFolder}/${savedFileName}`;
                    ...
                    console.error('[SAC] Erro ao salvar anexo do ticket interno:', e.message);
                }

                // Envia a notificação via WhatsApp (best effort: mesmo se falhar, o anexo já está no ticket)
                const evolutionUrl = `${apiUrl}/message/sendMedia/${instanceName}`;
                const payload = {
                    number: telFormat(telefone),
                    mediatype,
                ...
                try {
                    ...
                    console.error('[SAC] Erro Evolution API Media Internal (notificação):', e.message);
                }
            } else {
...
router.get('/atendentes', async (req, res) => {
    ...
    try {
        ...
        const sql = `
            ...
            UNION
            SELECT E.MATRICULA AS CODUSUR, NVL(E.AREAATUACAO, E.NOME) AS NOME, E.CELULAR AS TELEFONE1, E.FONE AS TELEFONE2
            FROM PCEMPR E
            WHERE E.SITUACAO = 'A'
...
router.post('/tickets/:id/avisar', async (req, res) => {
    ...
    try {
        ...
        // 2b. Marca o ticket como mencionado/referenciado para o atendente (ordena primeiro na lista dele)
        // Usa formato CODUSUR:CARGO para diferenciar VENDEDOR (PCUSUARI) de ATENDENTE (PCEMPR)
        try {
            let cargoAtendente = 'VENDEDOR';
            try {
                cargoAtendente = await determinarCargoAuto(conn, atendenteCod);
            } catch (cargoErr) {
                console.error('[SAC] Erro ao determinar cargo do atendente:', cargoErr.message);
            }
            await conn.execute(`
                UPDATE CANAL_SAC_TICKETS
                SET MENTIONED_MATRICULA = :m, MENTIONED_NOME = :n, ATUALIZADO_EM = SYSDATE
                WHERE ID = :id
            `, {
                m: String(atendenteCod || '') + ':' + cargoAtendente,
                n: atendenteNome || '',
                id
            }, { autoCommit: true });
        } catch (e) {
            console.error('[SAC] Erro ao marcar mencao no ticket:', e.message);
        }
