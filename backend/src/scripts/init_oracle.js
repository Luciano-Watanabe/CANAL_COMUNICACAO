...
const TABLES = [
    ...
    `CREATE TABLE CANAL_STATUS_WHATS (
        ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        TIPO_MIDIA VARCHAR2(20),               -- 'texto', 'imagem', 'video', 'audio'
        ARQUIVO_PATH VARCHAR2(255),
        LEGENDA VARCHAR2(4000),
        DATA_PROGRAMADA TIMESTAMP,
        VENDEDORES_DESTINO CLOB,               -- 'TODOS' ou JSON array de codusur
        CRIADO_POR VARCHAR2(50),
        STATUS_ENVIO VARCHAR2(50) DEFAULT 'PENDENTE',
    ...
    `CREATE TABLE CANAL_SAC_TICKETS (
        ...
        AGENDAMENTO_CODPROD NUMBER,
        AGENDAMENTO_QTDE NUMBER,
        AGENDAMENTO_MOTORISTA_NOME VARCHAR2(255),
        AGENDAMENTO_MOTORISTA_TEL VARCHAR2(50),
        AGENDAMENTO_ENVIADO CHAR(1) DEFAULT 'N'
    )`,
    `CREATE TABLE CANAL_SAC_TICKETS_MSGS (
        ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        TICKET_ID NUMBER NOT NULL,
        ENVIADO_POR VARCHAR2(50) NOT NULL,
        MENSAGEM CLOB NOT NULL,
        DATA_HORA TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        FOREIGN KEY (TICKET_ID) REFERENCES CANAL_SAC_TICKETS(ID) ON DELETE CASCADE
    )`,
    ...
    `CREATE TABLE CANAL_WEBHOOK_CONFIG (
        ID NUMBER PRIMARY KEY,
        PORTA NUMBER,
        TOKEN VARCHAR2(255),
        ATIVO CHAR(1) DEFAULT 'N',
        URL_PUBLICA VARCHAR2(500)
    )`,
    ...
    `CREATE TABLE CANAL_BOT_MENSAGENS (
        CHAVE VARCHAR2(100) PRIMARY KEY,
        DESCRICAO VARCHAR2(500),
        GRUPO VARCHAR2(100),
        BOT_TIPO VARCHAR2(20) DEFAULT 'SAC',
        TEMPLATE CLOB NOT NULL,
        ATUALIZADO_EM DATE DEFAULT SYSDATE
    )`,
    `CREATE TABLE CANAL_PESQUISA_PRECO (
        ID NUMBER GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
        DATA_HORA TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        TELEFONE_REMETENTE VARCHAR2(50),
        LOCAL VARCHAR2(255),
        PRODUTO_NOME VARCHAR2(255),
        EAN VARCHAR2(50),
        CODPROD NUMBER,
        PRECO_ENCONTRADO NUMBER(10,2),
        PRECO_TABELA NUMBER(10,2),
        CUSTO NUMBER(10,2),
        IMAGEM_URL VARCHAR2(1000),
        CODUSUR VARCHAR2(50)
    )`,
    `CREATE TABLE CANAL_BOT_PESQUISA_STATE (
        TELEFONE VARCHAR2(50) PRIMARY KEY,
        ESTADO_ATUAL VARCHAR2(100),
        LOCAL_ATUAL VARCHAR2(255),
        DADOS_TEMPORARIOS CLOB,
        ATUALIZADO_EM TIMESTAMP DEFAULT CURRENT_TIMESTAMP
    )`
];
...
async function initializeOracleDatabase() {
    ...
    try {
        ...
        // Initialize native webhook config
        try {
            await conn.execute(`INSERT INTO CANAL_WEBHOOK_CONFIG (ID, PORTA, TOKEN, ATIVO) SELECT 1, 3005, '', 'N' FROM DUAL WHERE NOT EXISTS (SELECT 1 FROM CANAL_WEBHOOK_CONFIG WHERE ID = 1)`);
            await conn.commit();
        } catch(e) {}
