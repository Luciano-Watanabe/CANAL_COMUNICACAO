import { Trophy, Medal, ShoppingCart, DollarSign } from 'lucide-react';
import clsx from 'clsx';
import { usePrivacy } from '../contexts/PrivacyContext';

export const ClientRanking = ({ ranking }: { ranking: any[] }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {ranking.map((cliente, index) => {
        ...
        const maxVenda = Math.max(...ranking.map(r => r.vltotal || 0));
        const progressWidth = maxVenda > 0 ? ((cliente.vltotal || 0) / maxVenda) * 100 : 0;

        return (
          <div key={cliente.codcli} className={clsx(
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className={clsx(
                  ...
                  {maskData(cliente.nome)}
                </h4>
                <p className="text-xs text-slate-500 font-mono tracking-wider font-semibold">CÓD: {cliente.codcli}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-200/50 dark:border-slate-700/50">
              <div className="flex flex-col">
                ...
                <span className={clsx("font-black text-lg", isFirst ? "text-amber-600 dark:text-amber-400" : "text-emerald-600 dark:text-emerald-400")}>
                  {cliente.pedidos || 0}
                </span>
              </div>
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="font-black text-lg text-primary-600 dark:text-primary-400">
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(cliente.vltotal || 0)}
                </span>
              </div>
            </div>
              ...
              <div className="w-full bg-slate-200 dark:bg-slate-700 rounded-full h-1.5 mt-2">
                <div 
                  className={clsx("h-1.5 rounded-full transition-all duration-500", 
                    ...
                    progressWidth > 40 ? "bg-amber-500" : "bg-primary-500"
                  )} 
