import { Trophy, Medal, Package, DollarSign } from 'lucide-react';
import clsx from 'clsx';
import { usePrivacy } from '../contexts/PrivacyContext';

export const ProductRanking = ({ ranking }: { ranking: any[] }) => {
  ...
  return (
    <div className="flex flex-col gap-4">
      {ranking.map((produto, index) => {
        ...
        const maxVenda = Math.max(...ranking.map(r => r.qtd_vendida || 0));
        const progressWidth = maxVenda > 0 ? ((produto.qtd_vendida || 0) / maxVenda) * 100 : 0;

        return (
          <div key={produto.codprod} className={clsx(
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h4 className={clsx(
                  ...
                  {maskData(produto.descricao)}
                </h4>
                <p className="text-xs text-slate-500 font-mono tracking-wider font-semibold">CÓD: {produto.codprod}</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between mt-2 pt-3 border-t border-slate-200/50 dark:border-slate-700/50">
              ...
              <div className="flex flex-col items-end">
                ...
                <span className="font-black text-lg text-primary-600 dark:text-primary-400">
                  {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(produto.vltotal || 0)}
                </span>
              </div>
            </div>
